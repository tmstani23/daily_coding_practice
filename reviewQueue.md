...
2. map teach-first (#34, done) -> map exercise (#35, done — primed solve, unprimed rep still owed) -> `||` vs `if` decision drill (#37, done — drill 2/2 cold, `??`/falsy gap found) -> unprimed map-selection rep (goal-only spec, no map talk beforehand) <- NEXT non-review slot
